...
| 2026-05-30 | — | refactor: 重构系统设置，取消独立全局配置，API 继承进具体模型/工具编辑；支持动态渲染真实生效的掩码 API Key；修复侧边栏切换导致会话模型重置回 Kimi 的 Bug 并跑通单元测试；支持对话栏标题在第一条消息后通过 AI 自动总结重命名，并支持手动修改 | Antigravity | — |
